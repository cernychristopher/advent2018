object Advent2 {
  ...
  def distance(s: String, t: String): Int = s.zip(t).map { case (c1, c2) => if (c1==c2) 0 else 1 }.sum

  def main(args: Array[String]): Unit = {
    val charCounts = testInput.map { str =>
      str.groupBy(identity).map(_._2.length).toSet
    }

    val twoCount = charCounts.count(_.contains(2))
    val threeCount = charCounts.count(_.contains(3))

    val solution1 = twoCount * threeCount
    println(solution1)
    ...
    val solution2 = (
      for {
        ...
        t <- testInput
        if distance(s,t) == 1
      } yield s.toSet.intersect(t.toSet)
    ).headOption
